package cn.deepai.evillage.model.bean;

/**
 * @author GaoYixuan
 */
public class TzjbxxBean {

    private String hid;
    private String zrrid;
    private String tznd;
    private String xm;
    private String nrjcsr;
    private String nsrhj;
    private String nzchj;
    private String dkje;
    private String zfjg;
    private String zfmj;
    private String sfwf;
    private String gdtian;
    private String gdtu;
    private String gdldm;
    private String gdhjm;
    private String sfts;
    private String sftd;
    private String sftl;
    private String sftx;
    private String sftds;
    private String sftkd;

    public String getHid() {
        return hid;
    }

    public void setHid(String hid) {
        this.hid = hid;
    }

    public String getZrrid() {
        return zrrid;
    }

    public void setZrrid(String zrrid) {
        this.zrrid = zrrid;
    }

    public String getTznd() {
        return tznd;
    }

    public void setTznd(String tznd) {
        this.tznd = tznd;
    }

    public String getXm() {
        return xm;
    }

    public void setXm(String xm) {
        this.xm = xm;
    }

    public String getNrjcsr() {
        return nrjcsr;
    }

    public void setNrjcsr(String nrjcsr) {
        this.nrjcsr = nrjcsr;
    }

    public String getNsrhj() {
        return nsrhj;
    }

    public void setNsrhj(String nsrhj) {
        this.nsrhj = nsrhj;
    }

    public String getNzchj() {
        return nzchj;
    }

    public void setNzchj(String nzchj) {
        this.nzchj = nzchj;
    }

    public String getDkje() {
        return dkje;
    }

    public void setDkje(String dkje) {
        this.dkje = dkje;
    }

    public String getZfjg() {
        return zfjg;
    }

    public void setZfjg(String zfjg) {
        this.zfjg = zfjg;
    }

    public String getZfmj() {
        return zfmj;
    }

    public void setZfmj(String zfmj) {
        this.zfmj = zfmj;
    }

    public String getSfwf() {
        return sfwf;
    }

    public void setSfwf(String sfwf) {
        this.sfwf = sfwf;
    }

    public String getGdtian() {
        return gdtian;
    }

    public void setGdtian(String gdtian) {
        this.gdtian = gdtian;
    }

    public String getGdtu() {
        return gdtu;
    }

    public void setGdtu(String gdtu) {
        this.gdtu = gdtu;
    }

    public String getGdldm() {
        return gdldm;
    }

    public void setGdldm(String gdldm) {
        this.gdldm = gdldm;
    }

    public String getGdhjm() {
        return gdhjm;
    }

    public void setGdhjm(String gdhjm) {
        this.gdhjm = gdhjm;
    }

    public String getSfts() {
        return sfts;
    }

    public void setSfts(String sfts) {
        this.sfts = sfts;
    }

    public String getSftd() {
        return sftd;
    }

    public void setSftd(String sftd) {
        this.sftd = sftd;
    }

    public String getSftl() {
        return sftl;
    }

    public void setSftl(String sftl) {
        this.sftl = sftl;
    }

    public String getSftx() {
        return sftx;
    }

    public void setSftx(String sftx) {
        this.sftx = sftx;
    }

    public String getSftds() {
        return sftds;
    }

    public void setSftds(String sftds) {
        this.sftds = sftds;
    }

    public String getSftkd() {
        return sftkd;
    }

    public void setSftkd(String sftkd) {
        this.sftkd = sftkd;
    }
}
